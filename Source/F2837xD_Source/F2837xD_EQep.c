...
{
//    // Initialize eQEP1
////eQEP1 for SNU encoder
//	// QDECCTL
//	//EQep1Regs.QDECCTL.bit.QSRC=0;	// 0: QEP quadrature count mode, general incremental encoder  (default : 0)
//	//EQep1Regs.QDECCTL.bit.SOEN=0;	// Sync output-enable, 0: Disable position-compare sync output (default : 0)
//	//EQep1Regs.QDECCTL.bit.SPSEL=0;	// Sync output pin selection, 0: Index pin is used for sync output (default : 0)
//	//EQep1Regs.QDECCTL.bit.XCR=0;	// External clock rate, 0: 2x resolution: Count the rising/falling edge (default : 0)
//	EQep1Regs.QDECCTL.bit.SWAP=0;	// Swap quadrature clock inputs, 1: Quadrature-clock inputs are swapped (default : 0)
//	//EQep1Regs.QDECCTL.bit.IGATE=0;	// Index pulse gating option, 0: Disable gating of Index pulse (default : 0)
//	//EQep1Regs.QDECCTL.bit.QAP=0;	// QEPA input polarity, 0: No effect (default : 0)
//	//EQep1Regs.QDECCTL.bit.QBP=0;	// QEPB input polarity, 0: No effect (default : 0)
//	//EQep1Regs.QDECCTL.bit.QIP=0;	// QEPI input polarity, 0: No effect (default : 0)
//	//EQep1Regs.QDECCTL.bit.QSP=0;	// QEPS input polarity, 0: No effect (default : 0)
//	// 4-0 bit are Reserved Always write as 0
//
//	// QEPCTL
//	EQep1Regs.QEPCTL.bit.FREE_SOFT=2;	// Emulation Control Bits, 2: unaffected by emulation suspend (default : 0)
//	EQep1Regs.QEPCTL.bit.PCRM=1;// Position counter reset mode, 1: Position counter reset on the maximum position (default : 0)
////	EQep1Regs.QEPCTL.bit.SEI=0;	// Strobe event initialization of position counter, 0x0: Does nothing (action disabled) (default : 0)
////	EQep1Regs.QEPCTL.bit.IEI=0;	// Index event initialization of position counter, 0x0: Do nothing (action disabled) (default : 0)
//	EQep1Regs.QEPCTL.bit.SWI=0;	// Software initialization of position counter, 0: Do nothing (action disabled) (default : 0)
////	EQep1Regs.QEPCTL.bit.SEL=0;	// Strobe event latch of position counter, 0: The position counter is latched on the rising edge of QEPS strobe (default : 0)
//	EQep1Regs.QEPCTL.bit.IEL=3;	// Index event latch of position counter, 3: Software index marker. (default : 0)
//	EQep1Regs.QEPCTL.bit.QPEN=1;// Quadrature position counter enable/software reset, 1: eQEP position counter is enabled (default : 0)
////	EQep1Regs.QEPCTL.bit.QCLM=0;// eQEP capture latch mode, 0: Latch on position counter read by CPU (default : 0)
//	EQep1Regs.QEPCTL.bit.UTE=1; // eQEP unit timer enable, 1: Enable unit timer (default : 0)
////	EQep1Regs.QEPCTL.bit.WDE=0; // eQEP watchdog enable, 0: Disable the eQEP watchdog timer (default : 0)
//
//	// QPOSCTL is not used
//	// QCAPCTL
//	EQep1Regs.QCAPCTL.bit.CEN=1; 	// Enable eQEP capture, 1: eQEP capture unit is enabled (default : 0)
////	EQep1Regs.QCAPCTL.bit.CCPS=0;	// eQEP capture timer clock prescaler, 0: CAPCLK = SYSCLKOUT/1  (default : 0)
////	EQep1Regs.QCAPCTL.bit.UPPS=0;   // Unit position event prescaler, 0: UPEVNT = QCLK/1 (default : 0)
//
//	// QPOSCNT, QPOSINIT are not needed to be initialized.
//	//EQep1Regs.QPOSMAX=encoder.ppr*encoder.mode-1; // This register contains the maximum position counter value. (0 ~ PPR*MODE - 1)
//		//-> InitEnc()에서 초기화
//
//	// QPOSCMP, QEINT are not used
//	EQep1Regs.QUPRD=0xFFFFFFFF;

//	InitEQep1Gpio(); -> 필요한건 이미 다 설정되어 있음


//eQEP2 for HD encoder
	// QDECCTL
	//EQep2Regs.QDECCTL.bit.QSRC=0;	// 0: QEP quadrature count mode, general incremental encoder (default : 0)
	//EQep2Regs.QDECCTL.bit.SOEN=0;	// Sync output-enable, 0: Disable position-compare sync output (default : 0)
	//EQep2Regs.QDECCTL.bit.SPSEL=0;	// Sync output pin selection, 0: Index pin is used for sync output (default : 0)
	//EQep2Regs.QDECCTL.bit.XCR=0;	// External clock rate, 0: 2x resolution: Count the rising/falling edge (default : 0)
	//EQep2Regs.QDECCTL.bit.SWAP=1;	// Swap quadrature clock inputs, 1: Quadrature-clock inputs are swapped (default : 0)
	//EQep2Regs.QDECCTL.bit.IGATE=0;	// Index pulse gating option, 0: Disable gating of Index pulse (default : 0)
	//EQep2Regs.QDECCTL.bit.QAP=0;	// QEPA input polarity, 0: No effect (default : 0)
	//EQep2Regs.QDECCTL.bit.QBP=0;	// QEPB input polarity, 0: No effect (default : 0)
	//EQep2Regs.QDECCTL.bit.QIP=0;	// QEPI input polarity, 0: No effect (default : 0)
	//EQep2Regs.QDECCTL.bit.QSP=0;	// QEPS input polarity, 0: No effect (default : 0)
	// 4-0 bit are Reserved Always write as 0

	// QEPCTL
	EQep2Regs.QEPCTL.bit.FREE_SOFT=2;	// Emulation Control Bits, 2: unaffected by emulation suspend  (default : 0)
	EQep2Regs.QEPCTL.bit.PCRM=1;// Position counter reset mode, 1: Position counter reset on the maximum position (default : 0)
	//EQep2Regs.QEPCTL.bit.SEI=0;	// Strobe event initialization of position counter, 0x0: Does nothing (action disabled) (default : 0)
	//EQep2Regs.QEPCTL.bit.IEI=0;	// Index event initialization of position counter, 0x0: Do nothing (action disabled) (default : 0)
	//EQep2Regs.QEPCTL.bit.SWI=0;	// Software initialization of position counter, 0: Do nothing (action disabled) (default : 0)
	//EQep2Regs.QEPCTL.bit.SEL=0;	// Strobe event latch of position counter, 0: The position counter is latched on the rising edge of QEPS strobe (default : 0)
	EQep2Regs.QEPCTL.bit.IEL=3;	// Index event latch of position counter, 3: Software index marker. (default : 0)
	EQep2Regs.QEPCTL.bit.QPEN=1;// Quadrature position counter enable/software reset, 1: eQEP position counter is enabled (default : 0)
	//EQep2Regs.QEPCTL.bit.QCLM=0;// eQEP capture latch mode, 0: Latch on position counter read by CPU (default : 0)
	EQep2Regs.QEPCTL.bit.UTE=1; // eQEP unit timer enable, 1: Enable unit timer (default : 0)
	//EQep2Regs.QEPCTL.bit.WDE=0; // eQEP watchdog enable, 0: Disable the eQEP watchdog timer (default : 0)

	// QPOSCTL is not used
	// QCAPCTL
	EQep2Regs.QCAPCTL.bit.CEN=1; 	// Enable eQEP capture, 1: eQEP capture unit is enabled (default : 0)
	EQep2Regs.QCAPCTL.bit.CCPS=5;	// eQEP capture timer clock prescaler, 0: CAPCLK = SYSCLKOUT/1 (default : 0) --> 5h = CAPCLK = SYSCLKOUT/32
	//EQep2Regs.QCAPCTL.bit.UPPS=0;   // Unit position event prescaler, 0: UPEVNT = QCLK/1 (default : 0)

	// QPOSCNT, QPOSINIT are not needed to be initialized.
	//EQep2Regs.QPOSMAX=encoder.ppr*encoder.mode-1; // This register contains the maximum position counter value. (0 ~ PPR*MODE - 1)
		//-> InitEnc()에서 초기화

	// QPOSCMP, QEINT are not used
	EQep2Regs.QUPRD=0xFFFFFFFF;
    //tbd...
